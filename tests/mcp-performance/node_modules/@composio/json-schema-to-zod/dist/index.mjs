...
function parseAllOf(jsonSchema, refs) {
	if (jsonSchema.allOf.length === 0) return z.never();
...
const parseAnyOf = (jsonSchema, refs) => {
	...
	}) : z.union(jsonSchema.anyOf.map((schema, i) => parseSchema(schema, {
		...refs,
		path: [
			...refs.path,
			"anyOf",
...
const parseArray = (jsonSchema, refs) => {
	...
	if (Array.isArray(jsonSchema.items)) return z.tuple(jsonSchema.items.map((v, i) => parseSchema(v, {
		...refs,
		path: [
			...refs.path,
			"items",
	...
	let zodSchema = !jsonSchema.items ? z.array(z.any()) : z.array(parseSchema(jsonSchema.items, {
		...refs,
...
const parseEnum = (jsonSchema) => {
	if (jsonSchema.enum.length === 0) return z.never();
	if (jsonSchema.enum.length === 1) return z.literal(jsonSchema.enum[0]);
	if (jsonSchema.enum.every((x) => typeof x === "string")) return z.enum(jsonSchema.enum);
...
const parseMultipleType = (jsonSchema, refs) => {
	return z.union(jsonSchema.type.map((type) => parseSchema({
		...jsonSchema,
		type
	}, refs)));
};
...
const parseOneOf = (jsonSchema, refs) => {
	if (!jsonSchema.oneOf.length) return z.any();
...
function parseObjectProperties(objectSchema, refs) {
	if (!objectSchema.properties) return z$1.object({});
	const propertyKeys = Object.keys(objectSchema.properties);
	if (propertyKeys.length === 0) return z$1.object({});
	const properties = {};
...
function parseObject(objectSchema, refs) {
	...
	if (normalizedSchema.patternProperties) {
		...
		if (propertiesSchema) if (additionalProperties) zodSchema = propertiesSchema.catchall(z$1.union([...patternPropertyValues, additionalProperties]));
		else if (Object.keys(parsedPatternProperties).length > 1) zodSchema = propertiesSchema.catchall(z$1.union(patternPropertyValues));
		else zodSchema = propertiesSchema.catchall(patternPropertyValues[0]);
		else if (additionalProperties) zodSchema = z$1.record(z$1.union([...patternPropertyValues, additionalProperties]));
		else if (patternPropertyValues.length > 1) zodSchema = z$1.record(z$1.union(patternPropertyValues));
		else zodSchema = z$1.record(patternPropertyValues[0]);
	...
	else if (additionalProperties) if (additionalProperties instanceof z$1.ZodNever) output = propertiesSchema.strict();
	...
	else if (additionalProperties) if (additionalProperties instanceof z$1.ZodNever) output = z$1.object({}).strict();
	else if (isAdditionalPropertiesTrue) output = z$1.object({}).passthrough();
	else output = z$1.record(additionalProperties);
	else output = z$1.object({}).passthrough();
...
const parseSchema = (jsonSchema, refs = {
	...
	if (typeof jsonSchema !== "object") return jsonSchema ? z$1.any() : z$1.never();
	if (refs.parserOverride) {
		const custom = refs.parserOverride(jsonSchema, refs);
		if (custom instanceof z$1.ZodType) return custom;
	}
	let seen = refs.seen.get(jsonSchema);
	if (seen) {
		if (seen.r !== void 0) return seen.r;
		if (refs.depth === void 0 || seen.n >= refs.depth) return z$1.any();
		seen.n += 1;
	} else {
		seen = {
			r: void 0,
...
//#endregion
export { jsonSchemaToZod, jsonSchemaToZodShape };