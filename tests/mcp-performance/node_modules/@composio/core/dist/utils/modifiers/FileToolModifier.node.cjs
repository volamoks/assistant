...
const require_FileToolModifier_utils_neutral = require('../../FileToolModifier.utils.neutral-Bcl7LVf4.cjs');

//#region src/errors/FileModifierErrors.ts
const FileModifierErrorCodes = { FILE_UPLOAD_FAILED: "FILE_UPLOAD_FAILED" };
var ComposioFileUploadError = class extends require_buffer.ComposioError {
...
const hydrateFiles = async (value, schema, ctx) => {
	if (schema?.file_uploadable) {
		...
		require_buffer.logger_default.debug(`Uploading file "${value}"`);
	...
	if (schemaVariants.length > 0) {
		const uploadableVariants = schemaVariants.filter(require_FileToolModifier_utils_neutral.schemaHasFileUploadable);
...
const downloadS3File = async (value, ctx) => {
	...
	try {
		require_buffer.logger_default.debug(`Downloading from S3: ${s3url}`);
		const dl = await require_fileUtils_node.downloadFileFromS3({
			toolSlug: ctx.toolSlug,
			s3Url: s3url,
			mimeType: mimetype ?? "application/octet-stream"
		});
		require_buffer.logger_default.debug(`Downloaded → ${dl.filePath}`);
		return {
			uri: dl.filePath,
		...
		require_buffer.logger_default.error(`Download failed: ${s3url}`, { cause: err });
		return {
			uri: "",
