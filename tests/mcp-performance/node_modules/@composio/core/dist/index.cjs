...
var ComposioConnectedAccountNotFoundError = class extends require_buffer.ComposioError {
...
var ComposioMultipleConnectedAccountsError = class extends require_buffer.ComposioError {
...
var ComposioFailedToCreateConnectedAccountLink = class extends require_buffer.ComposioError {
...
var ComposioProviderNotDefinedError = class extends require_buffer.ComposioError {
...
var ComposioToolNotFoundError = class extends require_buffer.ComposioError {
...
var ComposioCustomToolsNotInitializedError = class extends require_buffer.ComposioError {
...
var ComposioToolExecutionError = class extends require_buffer.ComposioError {
...
var ComposioInvalidExecuteFunctionError = class extends require_buffer.ComposioError {
...
* ```
*/
var ComposioToolVersionRequiredError = class extends require_buffer.ComposioError {
...
var ComposioAuthConfigNotFoundError = class extends require_buffer.ComposioError {
...
var ConnectionRequestTimeoutError = class extends require_buffer.ComposioError {
...
var ConnectionRequestFailedError = class extends require_buffer.ComposioError {
...
var ComposioToolkitNotFoundError = class extends require_buffer.ComposioError {
...
var ComposioToolkitFetchError = class extends require_buffer.ComposioError {
...
var ValidationError = class extends require_buffer.ComposioError {
...
var JsonSchemaToZodError = class extends require_buffer.ComposioError {
...
var ComposioNoAPIKeyError = class extends require_buffer.ComposioError {
...
var ComposioFailedToGetSDKRealtimeCredentialsError = class extends require_buffer.ComposioError {
...
var ComposioFailedToCreatePusherClientError = class extends require_buffer.ComposioError {
...
var ComposioFailedToSubscribeToPusherChannelError = class extends require_buffer.ComposioError {
...
var ComposioFailedToUnsubscribeFromPusherChannelError = class extends require_buffer.ComposioError {
...
var ComposioTriggerTypeNotFoundError = class extends require_buffer.ComposioError {
...
var ComposioWebhookSignatureVerificationError = class extends require_buffer.ComposioError {
...
var ComposioWebhookPayloadError = class extends require_buffer.ComposioError {
...
function transform(raw) {
	return { with(schema) {
		return { using(transformer, options) {
			...
			if (!result.success) {
				require_buffer.logger_default.error(result.error);
				return transformed;
			}
			return result.data;
		} };
...
function transformConnectedAccountResponse(response) {
	const parseState = (state) => {
		try {
			...
			require_buffer.logger_default.warn("Unsupported auth scheme in connected account state, ignoring state field", { error });
			return;
		}
	};
...
var CustomTools = class {
	...
	customToolsRegistry;
	constructor(client) {
		if (!client) throw new require_buffer.ComposioError("ComposioClient is required");
		this.client = client;
	...
	*/
	async getCustomToolBySlug(slug) {
		try {
			...
			require_buffer.logger_default.error(`Error getting custom tool: ${error}`);
			return;
		}
	}
	/**
	...
	*/
	async executeCustomTool(slug, body) {
		...
		if (toolkitSlug && toolkitSlug !== "custom" && body.userId) {
			...
			require_buffer.logger_default.debug(`[CustomTool] Connected account for ${toolkitSlug} found for user ${body.userId}`, JSON.stringify(connectedAccount, null, 2));
...
var Tools = class {
	...
	constructor(client, config) {
		...
		this.autoUploadDownloadFiles = config?.autoUploadDownloadFiles ?? _config_defaults.CONFIG_DEFAULTS.autoUploadDownloadFiles;
	...
	*/
	async applyDefaultSchemaModifiers(tools) {
		if (this.autoUploadDownloadFiles) {
			const fileToolModifier = new _file_tool_modifier.FileToolModifier(this.client);
	...
	*/
	async applyBeforeExecuteModifiers(tool, { toolSlug, toolkitSlug, params }, modifier) {
		...
		if (this.autoUploadDownloadFiles) modifiedParams = await new _file_tool_modifier.FileToolModifier(this.client).fileUploadModifier(tool, {
			toolSlug,
			toolkitSlug,
	...
	*/
	async applyAfterExecuteModifiers(tool, { toolSlug, toolkitSlug, result }, modifier) {
		...
		if (this.autoUploadDownloadFiles) modifiedResult = await new _file_tool_modifier.FileToolModifier(this.client).fileDownloadModifier(tool, {
			toolSlug,
			toolkitSlug,
	...
	*/
	async getRawComposioTools(query, options) {
		...
		require_buffer.logger_default.debug(`Fetching tools with filters: ${JSON.stringify(filters, null, 2)}`);
	...
	*/
	async getRawComposioToolBySlug(slug, options) {
		...
		if (customTool) {
			require_buffer.logger_default.debug(`Found ${slug} to be a custom tool`, JSON.stringify(customTool, null, 2));
			return customTool;
		} else require_buffer.logger_default.debug(`Tool ${slug} is not a custom tool. Fetching from Composio API`);
		let tool;
		try {
	...
	*/
	async function waitForConnection(timeout = 6e4) {
		try {
			...
			if (error instanceof _composio_client.default.NotFoundError) throw new ComposioConnectedAccountNotFoundError(`Connected account with id ${state.id} not found`, { meta: { connectedAccountId: state.id } });
			else throw error;
		}
...
var ConnectedAccounts = class {
	...
	*/
	async initiate(userId, authConfigId, options) {
		...
		else if (connectedAccount.items.length > 0) require_buffer.logger_default.warn(`[Warn:AllowMultiple] Multiple connected accounts found for user ${userId} in auth config ${authConfigId}`);
...
var Toolkits = class {
	...
	*/
	async getToolkitBySlug(slug) {
		try {
			...
			if (error instanceof openai.APIError && (error.status === 404 || error.status === 400)) throw new ComposioToolkitNotFoundError(`Toolkit with slug ${slug} not found`, {
				meta: { slug },
				cause: error
			});
	...
	*/
	...
	async getAuthConfigFields(toolkitSlug, authScheme, authConfigType, requiredOnly) {
		...
		if (toolkit.authConfigDetails.length > 1 && !authScheme) require_buffer.logger_default.warn(`Multiple auth configs found for ${toolkitSlug}, please specify the auth scheme to get details of specific auth scheme. Selecting the first scheme by default.`, { meta: { toolkitSlug } });
	...
	*/
	async authorize(userId, toolkitSlug, authConfigId) {
		...
		if (!authConfigIdToUse) if (toolkit.authConfigDetails && toolkit.authConfigDetails.length > 0) try {
			...
			if (error instanceof _composio_client.default.APIError && error.status === 400) throw new ComposioAuthConfigNotFoundError(`No Default auth config found for toolkit ${toolkitSlug}`, {
				meta: { toolkitSlug },
				cause: error,
...
const WebhookPayloadV3Schema = zod_v3.z.object({
	id: zod_v3.z.string(),
	timestamp: zod_v3.z.string(),
	type: zod_v3.z.literal("composio.trigger.message"),
	metadata: zod_v3.z.object({
		log_id: zod_v3.z.string(),
...
var InternalService = class {
	...
	*/
	async getSDKRealtimeCredentials() {
		...
		require_buffer.logger_default.debug(`[InternalService] SDK realtime credentials: ${JSON.stringify(parsedResponse, null, 2)}`);
...
var PusherService = class {
	...
	*/
	async getPusherClient() {
		if (!this.pusherClient) {
			...
			require_buffer.logger_default.debug(`[PusherService] Creating Pusher client for client ID: ${this.clientId} in cluster ${this.pusherCluster}`);
			try {
			...
			channel.bind("chunked-" + event, (data) => {
				try {
					...
					if (ev.receivedFinal && ev.chunks.length === Object.keys(ev.chunks).length) try {
						...
						require_buffer.logger_default.error("Failed to parse chunked data:", errorMessage);
					} finally {
						delete events[typedData.id];
					}
				} catch (error) {
					const errorMessage = error instanceof Error ? error.message : String(error);
					require_buffer.logger_default.error("Error processing chunked trigger data:", errorMessage);
					if (data && typeof data === "object" && "id" in data) delete events[data.id];
				}
			});
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : String(error);
			require_buffer.logger_default.error("Failed to bind chunked events:", error);
	...
	*/
	async subscribe(fn) {
		try {
			require_buffer.logger_default.debug(`[PusherService] Subscribing to channel: ${this.pusherChannel}`);
			...
			const safeCallback = (data) => {
				try {
					...
					const errorMessage = error instanceof Error ? error.message : String(error);
					require_buffer.logger_default.error("❌ Error in trigger callback:", errorMessage);
				}
			};
			this.bindWithChunking(channel, "trigger_to_client", safeCallback);
			require_buffer.logger_default.info(`✅ Subscribed to triggers. You should start receiving events now.`);
		} catch (error) {
	...
	*/
	async unsubscribe() {
		try {
			require_buffer.logger_default.debug(`[PusherService] Unsubscribing from channel: ${this.pusherChannel}`);
			await (await this.getPusherClient()).unsubscribe(this.pusherChannel);
			require_buffer.logger_default.info(`✅ Unsubscribed from triggers.`);
		} catch (error) {
...
var Triggers = class {
	...
	*/
	async create(userId, slug, body) {
		...
		try {
			...
			if (error instanceof _composio_client.APIError && (error.status === 400 || error.status === 404)) throw new ComposioTriggerTypeNotFoundError(`Trigger type ${slug} not found`, {
				cause: error,
				possibleFixes: [
		...
		try {
			...
			if (!connectedAccountId) {
				...
				require_buffer.logger_default.warn(`[Warn] Multiple connected accounts found for user ${userId}, using the first one. Pass connectedAccountId to select a specific account.`);
			}
		} catch (error) {
			if (error instanceof _composio_client.APIError && [400, 404].includes(error.status)) throw new ComposioConnectedAccountNotFoundError(`No connected account found for user ${userId} for toolkit ${toolkitSlug}`, {
				cause: error,
				possibleFixes: [`Create a new connected account for user ${userId}`]
			});
			throw error;
	...
	*/
	shouldSendTriggerAfterFilters(filters, data) {
		if (filters.toolkits?.length && !filters.toolkits.map((toolkit) => toolkit.toLowerCase()).includes(data.toolkitSlug.toLowerCase())) {
			require_buffer.logger_default.debug("Trigger does not match toolkits filter", JSON.stringify(filters.toolkits, null, 2));
			return false;
		}
		if (filters.triggerId && filters.triggerId !== data.id) {
			require_buffer.logger_default.debug("Trigger does not match triggerId filter", JSON.stringify(filters.triggerId, null, 2));
			return false;
		}
		if (filters.connectedAccountId && filters.connectedAccountId !== data.metadata.connectedAccount.id) {
			require_buffer.logger_default.debug("Trigger does not match connectedAccountId filter", JSON.stringify(filters.connectedAccountId, null, 2));
			return false;
		}
		if (filters.triggerSlug?.length && !filters.triggerSlug.map((triggerSlug) => triggerSlug.toLowerCase()).includes(data.triggerSlug.toLowerCase())) {
			require_buffer.logger_default.debug("Trigger does not match triggerSlug filter", JSON.stringify(filters.triggerSlug, null, 2));
			return false;
		}
		if (filters.triggerData && filters.triggerData !== data.metadata.triggerData) {
			require_buffer.logger_default.debug("Trigger does not match triggerData filter", JSON.stringify(filters.triggerData, null, 2));
			return false;
		}
		if (filters.userId && filters.userId !== data.metadata.connectedAccount.userId) {
			require_buffer.logger_default.debug("Trigger does not match userId filter", JSON.stringify(filters.userId, null, 2));
			return false;
		}
		require_buffer.logger_default.debug("Trigger matches all filters", JSON.stringify(filters, null, 2));
		return true;
	}
	/**
	...
	*/
	async subscribe(fn, filters = {}) {
		...
		require_buffer.logger_default.debug("🔄 Subscribing to triggers with filters: ", JSON.stringify(filters, null, 2));
		await this.pusherService.subscribe((_data) => {
			require_buffer.logger_default.debug("Received raw trigger data", JSON.stringify(_data, null, 2));
			const parsedData = this.parsePusherPayload(_data);
			if (this.shouldSendTriggerAfterFilters(parsedFilters.data, parsedData)) try {
				fn(parsedData);
			} catch (error) {
				require_buffer.logger_default.error("❌ Error in trigger callback:", error);
			}
			else require_buffer.logger_default.debug("Trigger does not match filters", JSON.stringify(parsedFilters.data, null, 2));
		});
	}
	/**
	...
	*/
	parsePusherPayload(data) {
		...
		if (versionedResult.ok) {
			require_buffer.logger_default.debug(`Parsed Pusher payload as ${versionedResult.version} format`);
			return versionedResult.normalizedPayload;
		}
		const legacyData = data;
		if (legacyData.metadata?.nanoId && legacyData.appName) {
			require_buffer.logger_default.debug("Parsed Pusher payload as legacy TriggerData format");
			return transformIncomingTriggerPayload(legacyData);
		}
		require_buffer.logger_default.warn("Unknown Pusher payload format. Payload keys: " + Object.keys(data).join(", "));
...
const userDataPath = () => {
	try {
		...
		return _platform.platform.joinPath(homeDir, require_buffer.COMPOSIO_DIR, require_buffer.USER_DATA_FILE_NAME);
	} catch (_error) {
		require_buffer.logger_default.debug("Environment", `Unable to get user data path`);
		return null;
	}
};
/**
...
const getUserDataJson = () => {
	try {
		...
		require_buffer.logger_default.debug("Environment", "No user data file found");
		return {};
	}
};
function getSDKConfig(baseUrl, apiKey) {
	const { api_key: apiKeyFromUserConfig, base_url: baseURLFromUserConfig } = getUserDataJson();
	const baseURLParsed = baseUrl || require_buffer.getEnvVariable("COMPOSIO_BASE_URL") || baseURLFromUserConfig || require_buffer.DEFAULT_BASE_URL;
	const apiKeyParsed = apiKey || require_buffer.getEnvVariable("COMPOSIO_API_KEY") || apiKeyFromUserConfig || "";
	if (!apiKeyParsed) require_buffer.ComposioError.handleAndThrow(new ComposioNoAPIKeyError());
	require_buffer.logger_default.debug("Environment", `API Key: ${apiKeyParsed}`);
	require_buffer.logger_default.debug("Environment", `Base URL: ${baseURLParsed}`);
	return {
...
function getToolkitVersionsFromEnv(defaultVersions) {
	...
	const envPrefixedVersions = require_buffer.getEnvsWithPrefix(`COMPOSIO_TOOLKIT_VERSION_`);
...
async function checkForLatestVersionFromNPM(currentVersion) {
	try {
		...
		if (semver.default.gt(latestVersion, currentVersionFromPackageJson) && !require_buffer.IS_DEVELOPMENT_OR_CI) require_buffer.logger_default.info(`🚀 Upgrade available! Your composio-core version (${currentVersionFromPackageJson}) is behind. Latest version: ${latestVersion}.`);
	} catch (_error) {}
}
	...
	*/
	async handleAssistantMessage(userId, run, options, modifiers) {
		...
		return await Promise.all(tool_calls.map(async (tool_call) => {
			require_buffer.logger_default.debug(`Executing tool call: ${tool_call.id}`);
			const tool_response = await this.executeToolCall(userId, tool_call, options, modifiers);
			require_buffer.logger_default.debug(`Tool call ${tool_call.id} executed with response: ${tool_response}`);
			return {
...
const ToolRouterCreateSessionConfigSchema = zod_v3.default.object({
	...
	connectedAccounts: zod_v3.default.record(zod_v3.default.string(), zod_v3.default.string()).describe("The connected accounts to use in the tool router session. The key is the toolkit slug, the value is the connected account id.").default({}),
...
const ToolkitConnectionsDetailsSchema = zod_v3.default.object({
	...
	nextCursor: zod_v3.default.string().optional(),
	totalPages: zod_v3.default.number()
});
...
var Composio = class Composio {
	...
	*/
	constructor(config) {
		...
		if (require_buffer.IS_DEVELOPMENT_OR_CI) require_buffer.logger_default.debug(`Initializing Composio w API Key: [REDACTED] and baseURL: ${baseURLParsed}`);
		/**
		...
		*/
		this.config = {
			...
			autoUploadDownloadFiles: config?.autoUploadDownloadFiles ?? _config_defaults.CONFIG_DEFAULTS.autoUploadDownloadFiles,
		...
		*/
		this.client = new _composio_client.default({
			...
			logLevel: require_buffer.COMPOSIO_LOG_LEVEL
		});
...
exports.AuthConfigErrorCodes = AuthConfigErrorCodes;
...
exports.ComposioAuthConfigNotFoundError = ComposioAuthConfigNotFoundError;
exports.ComposioConnectedAccountNotFoundError = ComposioConnectedAccountNotFoundError;
exports.ComposioCustomCreateResponseSchema = ComposioCustomCreateResponseSchema;
exports.ComposioCustomToolsNotInitializedError = ComposioCustomToolsNotInitializedError;
exports.ComposioError = require_buffer.ComposioError;
exports.ComposioFailedToCreateConnectedAccountLink = ComposioFailedToCreateConnectedAccountLink;
exports.ComposioFailedToCreatePusherClientError = ComposioFailedToCreatePusherClientError;
exports.ComposioFailedToGetSDKRealtimeCredentialsError = ComposioFailedToGetSDKRealtimeCredentialsError;
exports.ComposioFailedToSubscribeToPusherChannelError = ComposioFailedToSubscribeToPusherChannelError;
exports.ComposioFailedToUnsubscribeFromPusherChannelError = ComposioFailedToUnsubscribeFromPusherChannelError;
...
exports.ComposioGlobalExecuteToolFnNotSetError = ComposioGlobalExecuteToolFnNotSetError;
exports.ComposioInvalidExecuteFunctionError = ComposioInvalidExecuteFunctionError;
exports.ComposioInvalidModifierError = ComposioInvalidModifierError;
...
exports.ComposioMultipleConnectedAccountsError = ComposioMultipleConnectedAccountsError;
exports.ComposioNoAPIKeyError = ComposioNoAPIKeyError;
exports.ComposioProvider = ComposioProvider;
exports.ComposioProviderNotDefinedError = ComposioProviderNotDefinedError;
exports.ComposioToolExecutionError = ComposioToolExecutionError;
exports.ComposioToolNotFoundError = ComposioToolNotFoundError;
exports.ComposioToolVersionRequiredError = ComposioToolVersionRequiredError;
exports.ComposioToolkitFetchError = ComposioToolkitFetchError;
exports.ComposioToolkitNotFoundError = ComposioToolkitNotFoundError;
exports.ComposioTriggerTypeNotFoundError = ComposioTriggerTypeNotFoundError;
exports.ComposioWebhookPayloadError = ComposioWebhookPayloadError;
exports.ComposioWebhookSignatureVerificationError = ComposioWebhookSignatureVerificationError;
exports.ConnectedAccountAuthConfigSchema = ConnectedAccountAuthConfigSchema;
exports.ConnectedAccountErrorCodes = ConnectedAccountErrorCodes;
...
exports.ConnectionRequestErrorCodes = ConnectionRequestErrorCodes;
exports.ConnectionRequestFailedError = ConnectionRequestFailedError;
exports.ConnectionRequestTimeoutError = ConnectionRequestTimeoutError;
...
exports.JsonSchemaToZodError = JsonSchemaToZodError;
exports.MCP = MCP;
...
exports.SDKErrorCodes = SDKErrorCodes;
exports.ToolErrorCodes = ToolErrorCodes;
...
exports.ToolkitErrorCodes = ToolkitErrorCodes;
...
exports.TriggerErrorCodes = TriggerErrorCodes;
...
Object.defineProperty(exports, 'constants', {
  enumerable: true,
  get: function () {
    return require_buffer.constants_exports;
  }
});
exports.createConnectionRequest = createConnectionRequest;
exports.handleToolExecutionError = handleToolExecutionError;
exports.jsonSchemaToZodSchema = jsonSchemaToZodSchema;
exports.logger = require_buffer.logger_default;
