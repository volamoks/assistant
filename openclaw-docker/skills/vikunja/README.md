...
**Стало**: Анализ задач Vikunja + **добавление рекомендаций по реализации**

```bash
# Получить открытые задачи
vikunja.sh list-by-status undone

# Для [BUG] priority=3: добавить рекомендацию по fix
vikunja.sh update <task_id> "[title]" "[description]\n\nРЕКОМЕНДАЦИЯ: [конкретный fix]"

# Для [IMPROVE] priority=2: добавить план реализации
vikunja.sh update <task_id> "[title]" "[description]\n\nРЕКОМЕНДАЦИЯ: [план по шагам]\nПриоритет: HIGH/MEDIUM/LOW"

# Для повторяющихся багов: пометить как CRITICAL
vikunja.sh update <task_id> "[CRITICAL] Заголовок" "[description + повторения N раз]\n\nРЕКОМЕНДАЦИЯ: [fix]"

# Создать config-change задачу
vikunja.sh create-for-project <id> "[CONFIG] Заголовок" "Описание\n\nФайл: [path]\nИзменение: [что]" "YYYY-MM-DD" 2
```

**Результат**: Каждая задача [BUG] и [IMPROVE] получает рекомендацию от architect-агента для weekly-review.
...
**Стало**: Отчёт из Vikunja **с рекомендациями от nightly-evolution**
...
1. [Заголовок] — Priority N
   💡 Рекомендация: [кратко из описания]
   Vikunja: #<task_id>

🔧 УЛУЧШЕНИЯ:
2. [Заголовок] — Priority N
   💡 План: [шаги из описания]
   Приоритет: [HIGH/MEDIUM/LOW]
   Vikunja: #<task_id>
