...
WORK_TRANSCRIPTS_DIR = VAULT_ROOT / "Work" / "Transcripts"
...
def is_within_working_hours() -> bool:
    # For testing: always return True to run regardless of actual time
    return True
...
def main(dry_run: bool = False, limit: int = None):
    ...
    for f in files:
                    ...
                    if data_link:
                        try:
                            ...
                            if resp.status_code == 200:
                                segments = resp.json()
                                print(f"  → Fetched {len(segments)} segments from S3")
                                break
                        except Exception as se:
                            print(f"  → Failed to download transcript: {se}")
                            break

        if not segments:
            print(f"  → No transcript available for {file_id[:8]}. Skipping.")
            skipped_no_transcript += 1
            continue

        transcript_text = " ".join(s.get("content", s.get("text", "")) for s in segments if s.get("content") or s.get("text"))
        print(f"  → Got transcript: {len(transcript_text)} chars, {len(segments)} segments")
        ...
        note_path = WORK_TRANSCRIPTS_DIR / note_filename
